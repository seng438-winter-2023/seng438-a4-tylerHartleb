...
Each member automated 2 test cases using Selenium from the 8 we designed as a group. For the mutation testing, as our test cases required very little changes one team member made the changes while the others documented the mutants and reported the results of the mutation testing.

# Difficulties encountered, challenges overcome, and lessons learned
    
We encountered some challenges during web testing. We encountered a bug in selenium where windows opened through links would not be recognized fortunately we were able to make use of the data-* attributes that Amazon had defined to extract the address it should be going to. Another challenge was the dynamic nature of the store page, this made testing the shopping cart difficult as products could become unavailable, or special offers would pop up preventing smooth navigation through the website.
